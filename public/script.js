...
// modified to pass data to render as an argument
  function renderItems(items) {
    ...
    items.forEach((item, index) => {
      ...
      row.innerHTML = `
        <td>${item.id}</td>
        <td contenteditable="true" oninput="updateItem(${index}, 'prop1', this.innerText)">${item.name}</td>
        <td contenteditable="true" oninput="updateItem(${index}, 'prop2', this.innerText)">${item.age}</td>
        <td contenteditable="true" oninput="updateItem(${index}, 'prop3', this.innerText)">${item.study}</td>
        <td>
